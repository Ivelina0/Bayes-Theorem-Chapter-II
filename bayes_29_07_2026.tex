...
```latex
\subsection{From inverse probability to modern Bayesian inference}

By the beginning of the twentieth century, the algebra behind inverse
probability was no longer the main difficulty. A more fundamental question
was how the \textbf{prior probability} should be chosen. In particular, a
uniform prior intended to represent ``ignorance'' need not remain uniform
after the unknown parameter is re-expressed in a different scale. R.~A.~Fisher
responded by developing an alternative theory of statistical inference based
on the \textbf{likelihood}, rather than assigning probabilities to unknown
parameters. His influential 1922 paper introduced a systematic theory involving
likelihood, sufficiency, efficiency and statistical information
\cite{fisher1922foundations}.

At approximately the same time, however, \textbf{Dorothy Wrinch and Harold
Jeffreys} were developing probability in the opposite direction: as a general
method for reasoning from observations to scientific hypotheses
\cite{wrinchjeffreys1919,wrinchjeffreys1921,wrinchjeffreys1923}.
Jeffreys eventually attempted to overcome one of the principal objections to
the older Laplacean approach by constructing more principled
\textbf{non-informative}, or what would now often be called
\textbf{objective/default}, priors. In his \textit{Theory of Probability}
(1939), he proposed a prior derived from Fisher's own measure of information
\cite{jeffreys1939,robert2009jeffreys}. For a single parameter $\theta$ this is
now written
...
and for several parameters the corresponding rule is
$\pi_J(\boldsymbol{\theta})\propto
\sqrt{\det I(\boldsymbol{\theta})}$.
The important feature is its invariance under smooth reparameterisation:
the prior does not depend simply on which mathematical scale happens to be
used to describe the unknown quantity \cite{robert2009jeffreys,ly2017fisher}.

Bayesian inference was also being extended beyond estimating an unknown
parameter to \textbf{comparing entire scientific hypotheses}. Wrinch and
Jeffreys had already argued that a general scientific law could be assigned
positive prior probability; in 1932 J.~B.~S.~Haldane gave an important
statistical construction combining a point probability for an exact
hypothesis with a continuous prior over alternatives
\cite{haldane1932,etzwagenmakers2017}. Jeffreys then developed Bayesian
significance tests explicitly in 1935 and systematically in his 1939 book
\cite{jeffreys1935,jeffreys1939}. In modern notation, two hypotheses are
compared through
...
\underbrace{\frac{P(D\mid H_1)}{P(D\mid H_0)}}_{\text{Bayes factor}},
\]

so that the observations measure how much the relative support for the two
hypotheses should change. Jeffreys did not use the modern term
``Bayes factor''; this terminology was introduced later by I.~J.~Good in
1958 \cite{good1958}. During the Second World War, Alan Turing independently
used closely related \textbf{factors in favour} and additive weights of
evidence in cryptanalysis, providing an important practical application of
this form of Bayesian reasoning \cite{turingcryptography}.

A further development was to place inference inside a broader theory of
\textbf{decision-making}. Abraham Wald's work from 1939 onward treated
estimation and hypothesis testing as problems in which an action is chosen
and evaluated by a loss or risk function \cite{wald1939,wald1950}. In a
Bayesian formulation, an action $a$ may be selected by minimising posterior
expected loss,
\[
a^*=\arg\min_a \mathbb{E}[L(a,\theta)\mid D],
\]
thereby extending Bayesian inference from the question ``what should we
believe?'' to ``what should we do given what we now believe?''

After the war, attention increasingly turned to making Bayesian calculations
practical. Raiffa and Schlaifer's \textit{Applied Statistical Decision Theory}
(1961) systematised \textbf{conjugate prior distributions}, for which the prior
and posterior belong to the same distributional family
\cite{raiffaschlaifer1961}. Conjugacy made many posterior calculations
analytically tractable, but more complicated models still required numerical
methods. The Metropolis algorithm of 1953 and Hastings' 1970 generalisation
provided the foundations for the Markov chain Monte Carlo methods that would
later transform Bayesian computation
...
\subsection{So what is modern-day Bayes' theorem?}

In modern probability textbooks, Bayes' theorem is usually introduced first
as a consequence of conditional probability. For two events $A$ and $B$, with
$P(B)>0$,
...
The theorem therefore allows us to reverse a conditional probability: from
the probability of observing $B$ when $A$ is true, $P(B\mid A)$, to the
probability that $A$ is true after observing $B$, $P(A\mid B)$.
Tijms develops Bayes' rule in this form from conditional probability before
turning to its use in statistical inference \cite[Ch.~8]{tijms2012}.

In modern Bayesian statistics, the same rule is usually written in terms of
an unknown quantity or parameter $\theta$ and observed data $x$:
...
Here $p(\theta)$ is the \textbf{prior}, representing what is known or assumed
about $\theta$ before observing the data; $p(x\mid\theta)$ is the
\textbf{likelihood}, describing how probable the observed data are for each
possible value of $\theta$; and $p(\theta\mid x)$ is the
\textbf{posterior}, representing what is known about $\theta$ after the data
have been observed.  In the concise form used throughout modern Bayesian
statistics,
...
The only difference between the discrete and continuous versions is how the
posterior is normalised.  If $\theta$ can take values
$\theta_1,\ldots,\theta_n$, then
...
If $\theta$ is continuous, the sum is replaced by an integral,

\begin{equation}
    f(\theta\mid x)
    =
    \frac{L(x\mid\theta)f_0(\theta)}
    {\displaystyle\int
    L(x\mid u)f_0(u)\,du},
\end{equation}

as described by Tijms in his treatment of continuous Bayesian statistics
\cite[pp.~428--430]{tijms2012}.  Thus, what is now called
\textit{Bayes' theorem} is not a new theorem replacing the work of Bayes or
Laplace.  Rather, it is the modern notation and general framework through
which their central idea---using observed evidence to revise probabilities
assigned to unknown quantities---is expressed.
```
...
For readers unfamiliar with the basic concepts of probability, a particularly
accessible introduction is Tijms' \textit{Understanding Probability}
\cite{tijms2012}. Chapter~8 develops conditional probability, the law of total
probability and Bayes' rule before introducing Bayesian statistics for the
discrete case. Chapter~13 later extends these ideas to continuous random
variables and continuous Bayesian models. For readers already comfortable
with these probability concepts, Gelman et al.'s
\textit{Bayesian Data Analysis} provides a natural next step, developing the
same ideas within a much broader framework for modern Bayesian inference
\cite{gelman2013bayesian}.

Modern introductions to Bayes' theorem usually begin with the
\textbf{discrete case}. Suppose that $B_1,\ldots,B_k$ represent all possible
hypotheses, explanations or parameter values under consideration. These are assumed to be \textbf{mutually exclusive}, so that no two can occur simultaneously, and \textbf{exhaustive}, so that one of them must occur. Let $A$ denote some observed evidence. 

Two basic results from probability are needed. First, \textbf{conditional probability}
is defined by
...
\begin{figure}[H] \centering \includegraphics[width=0.45\textwidth]{figs/Bayes_figs/law_total_probability_venn.png} \caption{Geometric illustration of the law of total probability. The sample space is partitioned into mutually exclusive events $B_1,\ldots,B_n$, and the probability of $A$ is obtained by adding its intersections with each part of the partition. Source: \href{https://qualityengineer.weebly.com/probability/total-probability-law}{Blog by Authors: Armin Nikzad and Fatima Mosallaei}.} \label{fig:total_probability} \end{figure}
...
In words, Bayes' theorem begins with probabilities assigned to the different
possible hypotheses $B_i$, asks how compatible the observed evidence $A$ is
with each of them, and then uses this information to revise their
probabilities after the evidence has been observed. In modern Bayesian
terminology this is often summarised as
...
The same idea extends naturally to \textbf{continuous} problems. In the
discrete case there is a finite or countable list of possibilities,
$B_1,B_2,\ldots$. In a continuous problem, however, an unknown quantity may
take any value within an interval. For example, an unknown probability might
take any value between $0$ and $1$, rather than being restricted to a list of
possible values. Such an unknown quantity is commonly represented by a
parameter such as $\theta$.



\newpage

Modern introductions to Bayes' theorem usually begin with the \textbf{discrete case}. Suppose that $B_1,\ldots,B_k$ represent all possible hypotheses or explanations or parameter values under consideration. These are assumed to be \textbf{mutually exclusive} and \textbf{exhaustive} "events": only one can occur, and one of them must occur. 

Let $A$ denote some observed evidence. Two basic results from probability are needed. First, conditional probability is defined by \[ P(B_i\mid A)=\frac{P(B_i\cap A)}{P(B)}. \] 


Second, the \textbf{law of total probability} says that the probability of observing $B$ can be found by considering all the different hypotheses under which $B$ could have occurred: 

\[ P(B) = \sum_{j=1}^{k}P(A\mid B_j)P(B_j). \] 



This can be pictured as dividing the event $A$ into the mutually exclusive pieces contributed by each possible hypothesis: 

\begin{figure}[H] \centering \includegraphics[width=0.45\textwidth]{figs/Bayes_figs/law_total_probability_venn.png} \caption{Geometric illustration of the law of total probability. The sample space is partitioned into mutually exclusive events $B_1,\ldots,B_n$, and the probability of $A$ is obtained by adding its intersections with each part of the partition. Source: \href{https://qualityengineer.weebly.com/probability/total-probability-law}{Blog by Authors: Armin Nikzad and Fatima Mosallaei}.} \label{fig:total_probability} \end{figure}



% Since \[ P(E\cap H_j)=P(E\mid H_j)P(H_j), \] adding these pieces together gives the total probability of observing $E$. 

Further details on conditional probability and the law of total probability can be found in Chapter~8 of Tijms \cite{tijms2012}. 

Substituting these results into conditional probability gives the modern discrete form of Bayes' theorem: 

\begin{equation} \boxed{ P(H_i\mid E) = \frac{P(E\mid H_i)P(H_i)} {\displaystyle \sum_{j=1}^{k}P(E\mid H_j)P(H_j)} }. \end{equation}

In words, Bayes' theorem begins with probabilities assigned to the different possible hypotheses, asks how compatible the observed evidence is with each of them, and then uses this information to revise their probabilities after the evidence has been observed. 


In modern Bayesian terminology this is often summarised as 

\begin{equation} \boxed{ \text{posterior} \propto \text{likelihood} \times \text{prior} }. \end{equation} 


The same idea extends naturally to \textbf{continuous} problems. In the discrete case there is a finite or countable list of possibilities, $H_1,H_2,\ldots$. In a continuous problem, however, an unknown quantity may take any value within an interval. For example, an unknown probability might take any value between $0$ and $1$, rather than being restricted to a list of possible values. Such an unknown quantity is commonly represented by a parameter such as $\theta$.
...
Since the intersection can also be obtained by $P(A\cap B_i)=P(A\mid B_i)P(B_i)$,  substituting the law of total probability into conditional probability gives the modern discrete form of \textbf{Bayes' theorem}:
