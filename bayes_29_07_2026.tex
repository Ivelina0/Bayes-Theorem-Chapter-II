...
Although inverse probability had already found applications in astronomy,
geodesy, insurance and scientific inference during the nineteenth and early
twentieth centuries \cite{mcgrayne2011,hacking1975,franklin2001}, the Second
World War produced one of the most remarkable practical applications of
Bayesian reasoning. At Bletchley Park, probability was no longer being used
only to analyse scientific observations: it became a tool for deciding between
competing hypotheses when the evidence was incomplete, continually changing,
and militarily urgent.

The British work did not begin from nothing. During the 1930s, the Polish
Cipher Bureau mathematicians Marian Rejewski, Jerzy Różycki and Henryk
Zygalski had made fundamental breakthroughs against the German Enigma
system. Rejewski used the mathematics of permutations to reconstruct the
machine's internal wiring, while the Polish team developed mechanical and
paper-based techniques for recovering Enigma settings. Shortly before the
outbreak of war in 1939, these discoveries were shared with British and French
cryptanalysts and became an important foundation for the subsequent work at
Bletchley Park \cite{hodges2014,hinsley1993}.

The increasingly mechanical character of cryptography also changed the type
of expertise that the British Government required. Alastair Denniston, head of
the Government Code and Cypher School, deliberately recruited mathematicians
from British universities, including Alan Turing (1912--1954), Gordon
Welchman (1906--1985) and Max Newman (1897--1984). Turing was assigned to
Hut~8, which concentrated on German Naval Enigma. Irving John ``Jack'' Good
(1916--2009), originally trained as a pure mathematician, joined Hut~8 in
1941 and later described himself as Turing's ``main statistical assistant''
during this period \cite{good1979turing}.

The remarkable feature of this episode is that neither Turing nor Good had
entered Bletchley Park as professional statisticians. The cryptanalytic
problems themselves forced them to develop, rediscover and adapt probabilistic
methods. Good would later become one of the major advocates of Bayesian
statistics, publishing \textit{Probability and the Weighing of Evidence} in
1950 \cite{good1950}. Much of the detailed statistical work carried out during
the war, however, remained secret for decades.

\subsubsection*{The problem: German Naval Enigma}

The Enigma machine transformed every typed letter by passing an electrical
signal through a plugboard and a sequence of wired rotors before illuminating
a ciphertext letter. Because the rotors moved as the machine was used, the
substitution changed from one letter to the next. Knowledge of the machine
itself was therefore insufficient: the cryptanalyst also needed to infer its
unknown settings.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{images/enigma_machine.png}
    \caption{A German Enigma cipher machine. For Naval Enigma, the unknown
    rotor order and settings formed part of the hidden configuration that
    Allied cryptanalysts attempted to recover from intercepted messages.}
    \label{fig:enigma-machine}
\end{figure}

The German Navy employed Enigma using particularly elaborate operating
procedures. A later history by GCHQ describes Naval Enigma as
``the hardest cryptanalytic problem facing Bletchley Park''
\cite{gchq2012turing}. Turing deliberately took responsibility for this
problem, building on both the earlier Polish work and the experience of
British cryptanalysts such as Dilly Knox.

The difficulty was not simply that there were many possible settings.
Bletchley Park possessed electromechanical \emph{Bombes} capable of rejecting
large numbers of impossible settings, but Bombe time itself was scarce.
Cryptanalysis therefore became a problem of deciding \emph{which possibilities
were sufficiently plausible to deserve further computation}.

Different forms of evidence were used throughout the attack on Enigma.
Probable pieces of plaintext, known as \emph{cribs}, could generate logical
constraints for Bombe searches, while traffic patterns, repeated message
structures, operator habits and captured cryptographic material provided
additional information \cite{hodges2014,hinsley1993}. For Naval Enigma,
Turing also developed a statistical procedure known as \emph{Banburismus}.
By comparing patterns occurring in intercepted messages, the cryptanalysts
could accumulate evidence about possible relative message positions and rotor
orders and eliminate sufficiently implausible alternatives before committing
scarce Bombe time \cite{simpson2017banburismus,zabell2012turing}.

Thus the problem was not merely

\begin{quote}
\itshape
``Can this particular key be disproved?''
\end{quote}
...
\begin{quote}
\itshape
``Of the competing explanations still consistent with what we have observed,
which should we now regard as the most plausible?''
\end{quote}

This is precisely where Bayesian hypothesis testing enters the story.

\subsubsection*{From Bayes' theorem to hypothesis testing}

Earlier applications of Bayes' theorem in this chapter concerned an unknown
quantity such as a probability $\theta$. At Bletchley Park, however, the
unknown could instead be represented by a collection of \emph{competing
hypotheses}.
...
\[
H_1 =
\{\text{a particular proposed rotor order or alignment is correct}\},
\]

and let $H_0$ represent a specified rival explanation. Before examining some
new evidence $E$, the two possibilities are assigned \emph{prior odds}
...
After observing $E$, Bayes' theorem gives the posterior odds
...
\label{eq:turing-posterior-odds}
\end{equation}
...
measures how much the observed evidence changes the relative support for the
two hypotheses. In modern terminology it is a \emph{Bayes factor} when the
probabilities in the numerator and denominator are the appropriate predictive
probabilities under the competing hypotheses.

The distinction between \textbf{simple} and \textbf{composite} hypotheses is
important. If $H_0$ and $H_1$ completely specify the probability model, then

\[
B_{10}
=
\frac{L(H_1;E)}{L(H_0;E)}
\]

is simply a likelihood ratio. Good later recalled that many of Turing's
wartime comparisons had exactly this form \cite{good1979turing}. If a
hypothesis instead contains further unknown parameters $\theta_j$, its
predictive probability must average over them:
...
\int
P(E\mid\theta_j,H_j)
\pi_j(\theta_j)\,d\theta_j,
\end{equation}

which gives the more general form of Bayesian hypothesis testing encountered
in modern statistics.

Turing himself described this reasoning remarkably explicitly. In his
wartime manuscript \textit{The Applications of Probability to Cryptography},
he wrote that when ``definite rival theories'' could be proposed,
probability could be used to ``decide between them''
\cite{turing1941cryptography}. He referred to Bayes' theorem as the
\emph{factor principle} and expressed it in words as
...
\text{prior odds}
\times
\text{factor supplied by the evidence}.
\end{equation}

This was not the invention of Bayesian hypothesis testing itself. Related
ideas had already appeared in the work of Wrinch, Jeffreys and Haldane, and
Jeffreys had developed systematic Bayesian tests before the war. Turing's
importance here was different: he transformed odds-based hypothesis
comparison into an operational method for solving cryptanalytic problems under
severe computational constraints.

\footnote{It would therefore be misleading to say that ``frequentist methods
were useless'' at Bletchley Park. Turing's calculations relied heavily on
empirical frequencies, including frequencies of letters and repeated patterns,
and likelihood-ratio methods also have an important role in frequentist
hypothesis testing. What was especially useful about the Bayesian formulation
was its ability to combine prior information with successive likelihood
evidence and produce updated odds for competing hypotheses. During the same
wartime period Abraham Wald independently developed frequentist sequential
analysis; Good later described Turing as another independent inventor of
sequential statistical reasoning \cite{good1979turing}.}

\subsubsection*{Decibans: measuring the weight of evidence}

The cryptanalytic setting produced a further practical difficulty. Evidence
did not arrive once in a single completed experiment. Instead, one clue was
followed by another, and each clue changed the relative plausibility of the
remaining hypotheses.

Suppose that evidence arrives sequentially as

\[
E_1,E_2,\ldots,E_m.
\]

Bayes' theorem can then be applied repeatedly:

\begin{equation}
\frac{P(H_1\mid E_1,\ldots,E_m)}
     {P(H_0\mid E_1,\ldots,E_m)}
=
\frac{P(H_1)}{P(H_0)}
\prod_{j=1}^{m}
\frac{
P(E_j\mid E_1,\ldots,E_{j-1},H_1)
}{
P(E_j\mid E_1,\ldots,E_{j-1},H_0)
}.
\label{eq:sequential-bayes-turing}
\end{equation}

If the pieces of evidence are conditionally independent given the hypothesis,
this simplifies to a product of ordinary likelihood ratios. Turing realised
that repeatedly multiplying many such numbers was inconvenient for manual
calculation. Taking logarithms converts multiplication into addition.

He therefore measured the evidence supplied by $E$ in favour of $H_1$ over
$H_0$ by
...
and called the resulting unit a \emph{deciban}.
...
Independent pieces of evidence could therefore simply be \emph{added}:
...
Ten decibans correspond to multiplying the odds by $10$, while one deciban
...
Positive decibans favour $H_1$; negative decibans favour $H_0$. Instead of
asking for an immediate yes-or-no decision after every observation,
cryptanalysts could therefore allow the evidence to accumulate until one
hypothesis had gained sufficient support to justify further work.

Good later described the deciban as one of Turing's wartime statistical
innovations and explained that the unit was connected with the large sheets
printed in the town of Banbury on which these weights of evidence were
recorded during Banburismus \cite{good1979turing}. In modern statistical
language, Turing was working with logarithmic Bayes factors and performing
sequential discrimination between competing hypotheses.

Joan Clarke (1917--1996), another mathematician in Hut~8, became particularly
adept at applying Banburismus in practice \cite{gchq2019clarke}. Her role is
therefore important to the operational history of the method, although the
surviving theoretical account of its probabilistic foundations is associated
primarily with Turing and, subsequently, Good. Hugh Alexander
(1909--1974) was likewise regarded as one of the most accomplished
practitioners of Banburismus and later led Hut~8.

\subsubsection*{From wartime secrecy to Bayesian statistics}

The statistical importance of this work was initially hidden by wartime
secrecy. Turing's own manuscript on the applications of probability to
cryptography remained classified for decades, while Good gradually developed
many of the underlying ideas in his post-war statistical work.

His 1950 book \textit{Probability and the Weighing of Evidence} developed an
explicit Bayesian approach to inference \cite{good1950}. In 1953 he published
\textit{The Population Frequencies of Species and the Estimation of Population
Parameters}, which developed work originating with Turing on estimating the
probability of rare and previously unseen events and became associated with
the modern Good--Turing estimator \cite{good1953population}.

Only much later could Good describe the wartime origins directly. His 1979
paper \textit{A.~M. Turing's Statistical Work in World War II} listed among
Turing's wartime contributions factors in favour of hypotheses, sequential
analysis, logarithmic weights of evidence, decibans, expected weight of
evidence, search trees, repeat rates and ideas later connected with empirical
Bayes methods \cite{good1979turing}. Turing's own declassified report provides
an even more direct historical source, showing Bayes' theorem, prior
probabilities, competing hypotheses and deciban calculations in his own
wartime exposition \cite{turing1941cryptography,zabell2012turing}.

The importance of the episode is therefore broader than the breaking of a
single cipher. Bayes' theorem had existed for nearly two centuries, and
Bayesian hypothesis testing had already been developing before Turing arrived
at Bletchley Park. What the wartime cryptanalytic problem demonstrated was how
these ideas could be turned into a practical system for \emph{ranking
hypotheses, accumulating evidence and allocating limited computational
resources}. In that sense, the mathematics of Banburismus provides an early
and unusually concrete example of Bayesian reasoning operating as a
