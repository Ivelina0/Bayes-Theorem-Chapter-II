...
(1862--1943) called for its axiomatization in his sixth problem of 1900;
Émile Borel (1871--1956) and Henri Lebesgue (1875--1941) connected probability
with the emerging theory of measure, while Richard von Mises (1883--1953)
developed an explicitly frequency-based interpretation. The mathematical
development culminated in Andrey Kolmogorov's (1903--1987)
...
But the question of inverse probability persisted: \textit{what exactly may be assigned a probability?} If $\theta$ is unknown but fixed, can we legitimately speak of $P(\theta\mid D)$? During the following decades of the 20-th century, several different answers were developed.

\paragraph{Fisher: likelihood instead of inverse probability (1922--1935).}

Ronald Aylmer Fisher (1890--1962) developed a non-Bayesian programme of statistical inference based on \textbf{likelihood}, \textbf{maximum likelihood estimation}, \textbf{significance testing} and later \textbf{fiducial inference}, together with the concepts of sufficiency, efficiency and statistical information \cite{fisher1922foundations,fisher1925statistical,fisher1930inverse, fisher1935fiducial,aldrich1997}. For Fisher, the data were random but the unknown parameter was fixed: likelihood could compare parameter values without assigning them prior probabilities. His rejection of inverse probability was explicit. In 1930 he described it as a doctrine that appeared to \begin{quote} ``a succession of sound writers to be \textit{fundamentally false and devoid of foundation}'' \cite[p.~528]{fisher1930inverse}. \end{quote}

At Rothamsted, Fisher worked closely with his first statistical assistant,
\textbf{Winifred Alice Mackenzie (1896--1954)}, who co-authored studies on
rainfall, bacterial counts and crop variation, including an important 1923
paper connected with the early development of analysis of variance
\cite{fishermackenzie1923,tayloraldrich2022}. Historians have additionally
pointed out that much of Rothamsted's hand computation was carried out by women
who rarely became visible in the usual history of statistics
\cite{parolini2015}.
...
non-Bayesian answer. Their work between 1928 and 1933 treated testing as a
choice between hypotheses and evaluated a procedure through its repeated
performance: \textbf{Type I error}, \textbf{Type II error} and
\textbf{power} \cite{neymanpearson1928,neymanpearson1933}.

Thus the question was no longer $P(H\mid D)=?$ but, roughly,

\[
P(\text{our procedure makes an error}\mid H)=?
\]

Neyman's 1937 theory of \textbf{confidence intervals} extended the same
repeated-sampling philosophy to estimation \cite{neyman1937}. Fisher and
Neyman--Pearson are therefore often grouped together today, but their
interpretations of statistical testing were not identical. Nevertheless, in modern times their combined "frequentist" approach provides hypothesis-testing problems that can be studied with the binomial and Normal distributions already available at A-level, without requiring an additional theory of prior distributions and posterior integration.

\paragraph{Wrinch, Haldane and Jeffreys: probabilities of hypotheses
(1919--1946).}

Meanwhile, Dorothy Wrinch (1894--1976) and Harold Jeffreys (1891--1989)
were developing almost the opposite programme. Between 1919 and 1923 they
treated probability as a logic for reasoning about scientific propositions
\cite{wrinchjeffreys1919,wrinchjeffreys1921,wrinchjeffreys1923}. In 1921 they wrote that

% They argued that evidence could alter the probability assigned not merely to a parameter but to an entire scientific law.

% In 1921 they wrote that
...
% Wrinch's contribution was substantial rather than incidental. Jeffreys later
% described their joint work as the basis of his subsequent programme of
% scientific inference \cite{hodgkinjeffreys1976}.

% J.~B.~S.~Haldane (1892--1964) supplied an important mathematical step in
% 1932 by allowing an exact hypothesis to receive positive prior probability
% while the alternatives were represented by a continuous distribution
% \cite{haldane1932,etzwagenmakers2017}. Jeffreys then developed Bayesian
% significance tests in 1935 and systematically in his
% \textit{Theory of Probability} (1939)
% \cite{jeffreys1935,jeffreys1939}.

J.~B.~S.~Haldane (1892--1964) introduced a prior combining positive probability on an exact hypothesis with a continuous distribution over alternatives \cite{haldane1932,etzwagenmakers2017}. Jeffreys then developed Bayesian significance tests in 1935 and systematically in \textit{Theory of Probability} (1939) \cite{jeffreys1935,jeffreys1939}.

% In modern notation the central comparison becomes
...
% The final ratio was later called the \textbf{Bayes factor}
% \cite{good1958}. Unlike a (frequentist) $p$-value, it asks directly how the observed evidence changes the relative support for two hypotheses.

Jeffreys also returned to the old difficulty of choosing a prior. His 1939
book developed invariant approaches to prior choice, and his 1946 paper gave
the general Fisher-information rule $\pi_J(\boldsymbol{\theta})
\propto \sqrt{\det I(\boldsymbol{\theta})},$ which remains invariant under smooth reparameterisation
\cite{jeffreys1939,jeffreys1946}.
...
% which remains invariant under smooth reparameterisation
% \cite{jeffreys1939,jeffreys1946}.

\paragraph{Wald and Savage: inference becomes decision-making
(1939--1954).}

Abraham Wald (1902--1950) reframed the problem again. From 1939 onward he
treated estimation and hypothesis testing as instances of a more general
\textbf{decision problem}: after observing data, a statistician must choose
some action $a$. This action is evaluated through a \textbf{loss function}
$L(a,\theta)$, which measures the cost of taking action $a$ when the true,
but unknown, model parameter is $\theta$ \cite{wald1939,wald1950}.

For example, an action might be an estimate of an unknown parameter, a
decision to accept or reject a hypothesis, or some other choice made from the
observed data. A \textbf{decision rule}, written $\delta(X)$, is therefore a
rule that tells us which action to take for each possible dataset $X$.

Because the observed data are random, the action $\delta(X)$ may also vary
from one sample to another. Wald therefore evaluated a decision rule through
its \textbf{risk function},
...
Here, $\mathbb{E}_{\theta}$ means that the loss is averaged over all possible
datasets that could be generated when the true parameter has the fixed value
$\theta$. Thus $R(\theta,\delta)$ measures the \textbf{long-run expected
loss} of using the rule $\delta$ when $\theta$ is the true parameter.

This framework was not intrinsically Bayesian. A frequentist could compare
decision rules according to their long-run risk
\cite{wald1945}. A Bayesian instead assigns a probability distribution to
the unknown parameter $\theta$ and updates this distribution after observing
the actual data $D$. The Bayesian then chooses the action with the smallest
\textbf{posterior expected loss},
...
Wald's sequential decision theory influenced Richard Bellman's (1920--1984) development of \textbf{dynamic programming}, extending decision-making to multi-stage problems \cite{wald1950,bellman1957}. Dynamic programming later became foundational to optimal control, Markov decision processes and reinforcement learning.
...
% Abraham Wald (1902--1950) reframed the problem again. From 1939 onward he
% treated estimation and testing as instances of a more general
% \textbf{decision problem}: an action $a$ is evaluated through a
% \textbf{loss function} $L(a,\theta)$, which measures the cost of taking
% action $a$ when the true model parameter is $\theta$ \cite{wald1939,wald1950}.

% For a decision rule $\delta(X)$, its \textbf{risk function} is the expected
% loss over repeated samples,
...
% This framework was not intrinsically Bayesian. A frequentist could evaluate
% the long-run risk of a decision rule \cite{wald1945}; a Bayesian could instead
...
% =
% \arg\min_a
% \mathbb{E}[L(a,\theta)\mid D].
% \]

% Abraham Wald (1902--1950) reframed the problem again. From 1939 onward he
% treated estimation and testing as instances of a more general
% \textbf{decision problem}: an action $a$ is chosen and evaluated through a loss function $L(a,\theta)$, that depends on a model parameter $\theta$ and its associated risk
% \cite{wald1939,wald1950}.

% This framework was not intrinsically Bayesian. A frequentist could evaluate the long-run risk of a decision rule \cite{wald1945}; a Bayesian could instead minimise posterior expected loss,
...
% Leonard J.~Savage (1917--1971) subsequently connected decision theory,
% subjective probability and Bayesian updating in
% \textit{The Foundations of Statistics} (1954)
% \cite{savage1954}.

% The Second World War also supplied practical examples: Alan Turing
% (1912--1954) used likelihood ratios and additive weights of evidence in
% cryptanalysis at Bletchley Park, work later described by I.~J.~Good
% (1916--2009) \cite{turing1941,good1979turing}. These applications are best
% seen here as evidence that probabilistic updating was already being used in
% practice rather than as another separate theoretical school.

% \paragraph{When did ``Bayesian'' and ``frequentist'' become names?
% (c.~1950 onward).}

% Interestingly, the statisticians above did not originally describe themselves
% using the neat modern opposition ``Bayesian versus frequentist''. They spoke
% instead of \textit{inverse probability}, likelihood, significance tests,
% confidence methods, prior probabilities and decision rules.

% The adjective \textbf{``Bayesian''} only entered statistical usage around
% 1950. Ironically, one of its earliest documented uses was by Fisher himself
% in notes accompanying his \textit{Contributions to Mathematical Statistics}
% (1950) \cite{fienberg2006,aldrich2008}. The label became increasingly common
% during the 1950s and 1960s as writers such as Good, Savage and Lindley
% developed an explicitly Bayesian statistical programme.

% The opposing label \textbf{``frequentist''} also became standard only later.
% There was no single meeting at which the two schools were officially named:
% the terminology emerged retrospectively as the differences between the
% approaches became clearer. Neyman himself was explicitly using the description
% ``frequentist probability and frequentist statistics'' by the 1970s
% \cite{neyman1977}. Thus it is slightly anachronistic to describe the
% Fisher--Jeffreys disputes of the 1920s and 1930s as a debate between people
% who already called themselves ``frequentists'' and ``Bayesians''.

% \paragraph{A legacy we still learn today.}

% The consequences of this history remain visible in elementary statistics.
% In present-day A-level Mathematics in England, students learn
% \textbf{null and alternative hypotheses}, \textbf{significance levels},
% binomial and Normal hypothesis tests, and the idea of rejecting a null
% hypothesis when an observed result would be sufficiently unlikely under it.
% These descend primarily from the Fisher and Neyman--Pearson branches of
% twentieth-century statistics.

% Bayesian counterparts---prior distributions, posterior probabilities,
% credible intervals and Bayes factors---are not part of the compulsory
% A-level Mathematics syllabus. This should not be read as meaning that one
% approach is mathematically ``correct'' and the other is not. Rather, the
% classical testing framework became deeply embedded in scientific practice,
% textbooks and consequently school curricula during the twentieth century.
% It also provides hypothesis-testing problems that can be studied with the
% binomial and Normal distributions already available at A-level, without
% requiring an additional theory of prior distributions and posterior
% integration.

% The history therefore gives an important warning: what students first meet as
% ``statistics'' is actually one branch of a much larger historical debate over
% what can be inferred from uncertain evidence.
...
% At the beginning of the twentieth century, probability itself was being placed
% on firmer mathematical foundations. Hilbert's sixth problem (1900) had called
% for an axiomatic treatment of probability; work by Borel and Lebesgue connected
% probability with measure theory, while von Mises developed a frequency
% interpretation based on repeated observations. These developments culminated
% mathematically in Kolmogorov's \textit{Grundbegriffe der
% Wahrscheinlichkeitsrechnung} (1933), which formulated probability using the
% axioms of a measure on a space of events
% \cite{hilbert1900,borel1909,vonmises1919,kolmogorov1933}.

% This mathematical clarification did not, however, settle a different question:
% \textit{what may legitimately be assigned a probability?} In particular, if
% $\theta$ is an unknown but fixed parameter, may we speak of
% $P(\theta\mid D)$ at all? It was over this question---rather than over the
% algebra of Bayes' theorem---that several competing theories of statistical
% inference developed.

% \paragraph{Fisher: likelihood instead of inverse probability.}

% R.~A.~Fisher's answer was largely \textbf{no}. In his 1922 paper
% \textit{On the Mathematical Foundations of Theoretical Statistics}, he
% separated \textbf{likelihood} from probability and developed maximum
% likelihood estimation together with the concepts of sufficiency, efficiency
% and statistical information \cite{fisher1922foundations,aldrich1997}.
% For Fisher, the observations were random while an unknown parameter was fixed;
% a likelihood function could therefore measure how strongly different parameter
% values were supported by the observations without turning those values into a
% probability distribution.

% His rejection of the older inverse-probability argument became explicit. In
% 1930 Fisher described inverse probability as a doctrine that had appeared to
% ``a succession of sound writers to be \textit{fundamentally false and devoid
% of foundation}'' \cite[p.~528]{fisher1930inverse}.

% Fisher consequently developed a different collection of inferential tools:
% \textbf{maximum likelihood} for estimating parameters and
% \textbf{significance tests} for assessing whether observations were unusually
% incompatible with a null hypothesis \cite{fisher1925statistical}. A small
% $p$-value was not the posterior probability that the null hypothesis was
% false. Fisher also developed \textbf{fiducial inference} in an attempt to make
% probability statements about unknown parameters without introducing a Bayesian
% prior \cite{fisher1930inverse,fisher1935fiducial}.

% \paragraph{Neyman and Pearson: errors and repeated decisions.}

% Jerzy Neyman and Egon Pearson developed another non-Bayesian theory. Their
% papers of 1928--1933 replaced Fisher's single significance test with an
% explicit comparison between a null hypothesis $H_0$ and an alternative
% $H_1$ \cite{neymanpearson1928,neymanpearson1933}. The central quantities were
% not posterior probabilities but the long-run behaviour of a testing rule:
% the probability of a \textbf{Type I error}, the probability of a
% \textbf{Type II error}, and the \textbf{power} of the test.

% The Neyman--Pearson lemma then identified most-powerful tests for simple
% hypotheses at a prescribed Type I error rate. Neyman's 1937 theory of
% \textbf{confidence intervals} extended the same repeated-sampling philosophy
% to estimation \cite{neyman1937}. Thus Fisher and Neyman--Pearson both rejected
% ordinary inverse probability, but their alternatives were themselves
% different: Fisher emphasised evidence supplied by the observed experiment,
% whereas Neyman and Pearson emphasised the long-run performance of a rule.

% \paragraph{Wrinch and Jeffreys: probabilities of scientific hypotheses.}

% A very different programme had already been developing at Cambridge.
% Between 1919 and 1923, mathematician \textbf{Dorothy Wrinch} and geophysicist
% \textbf{Harold Jeffreys} published a series of papers treating probability as
% a general logic of scientific inference
% \cite{wrinchjeffreys1919,wrinchjeffreys1921,wrinchjeffreys1923}. They argued
% that evidence could alter the probability assigned not merely to an unknown
% parameter but to a scientific law itself. In 1921 they wrote that
...
% Wrinch's role in this development is easily understated. After her death,
% Jeffreys explicitly acknowledged the ``substantial contribution'' she had
% made to their joint work, which he described as ``the basis of all my later
% work on scientific inference'' \cite{hodgkinjeffreys1976}.

% This idea required an exact scientific law---for example,
% $H_0:\theta=0$---to receive positive prior probability. J.~B.~S.~Haldane
% provided an important mathematical construction in 1932 by combining a point
% probability at the exact hypothesis with a continuous distribution over the
% remaining values of $\theta$ \cite{haldane1932,etzwagenmakers2017}. Jeffreys
% then developed explicit Bayesian significance tests in 1935 and a systematic
% theory in \textit{Theory of Probability} (1939)
% \cite{jeffreys1935,jeffreys1939}.

% Jeffreys described the inferential problem succinctly:

% \begin{quote}
% ``our problem is to compare a suggested value of a new parameter, often 0,
% with the aggregate of other possible values''
% \cite[p.~193]{jeffreys1939}.
% \end{quote}

% In modern notation this comparison is
...
% where the second ratio later became known as the
% \textbf{Bayes factor} \cite{good1958}. Thus Jeffreys' procedure answered a
% different question from a Fisherian $p$-value or a Neyman--Pearson test:
% it measured how the observed data changed the relative probability of two
% hypotheses.

% Jeffreys also addressed the older problem of how prior ignorance should be
% represented. His 1939 book developed invariant approaches to prior choice,
% and in 1946 he gave the general Fisher-information rule
% \cite{jeffreys1939,jeffreys1946},
...
% Unlike a prior chosen merely to be uniform on one particular scale, this rule
% transforms consistently under smooth reparameterisation.

% \paragraph{Wald: inference as a decision problem.}

% A fourth approach asked a different question again. Beginning in 1939,
% \textbf{Abraham Wald} treated estimation and hypothesis testing as
% \textbf{decision problems}: an investigator chooses an action, incurs a loss
% $L(a,\theta)$, and evaluates procedures through their risk
% \cite{wald1939,wald1950}. Decision theory was therefore not itself an
% alternative definition of probability and was not inherently Bayesian.

% Within a Bayesian analysis, however, the natural rule is to minimise posterior
% expected loss,
...
% =
% \arg\min_a
% \mathbb{E}\!\left[L(a,\theta)\mid D\right].
% \]

% Wald's framework could also be used without a prior through frequentist ideas
% such as minimax risk. Leonard Savage's
% \textit{The Foundations of Statistics} (1954) later connected decision theory
% much more explicitly with \textbf{subjective probability}, providing another
% foundation for Bayesian inference \cite{savage1954}.

% By the middle of the twentieth century there was therefore no single
% replacement for inverse probability. Fisher had developed
% \textbf{likelihood and significance testing}; Neyman and Pearson,
% \textbf{error-controlled hypothesis testing and confidence intervals};
% Wrinch, Haldane and Jeffreys, \textbf{probabilities of hypotheses, Bayesian
% testing and principled priors}; and Wald, a general
% \textbf{theory of statistical decisions}. The modern distinction between
% Bayesian and frequentist inference emerged from these competing answers to
% what probability could legitimately say about an unknown quantity after data
% had been observed.

% During the Second World War, Alan Turing also used odds and additive weights
% of evidence in cryptanalytic work at Bletchley Park; his statistical methods
% were later documented by I.~J.~Good
% \cite{turing1941,good1979turing}. This was an important application of the
% same probabilistic logic, rather than a separate theoretical development.
% ```
...
%   address   = {Oxford},
%   year      = {1939},
%   url       = {https://archive.org/details/in.ernet.dli.2015.2608}
% }

% @article{robert2009jeffreys,
...
% @article{ly2017fisher,
%   author  = {Ly, Alexander and Marsman, Maarten and Verhagen, Josine
%              and Grasman, Raoul and Wagenmakers, Eric-Jan},
...
% @misc{turingcryptography,
%   author = {Turing, Alan M.},
%   title  = {The Applications of Probability to Cryptography},
%   note   = {Second World War research paper; typeset version by Ian Taylor},
%   year   = {1941},
...
%   address   = {Boston},
%   year      = {1961},
%   url       = {https://books.google.com/books?id=SpO0KFcFQDsC}
% }

% @article{metropolis1953,
%   author  = {Metropolis, Nicholas and Rosenbluth, Arianna W.
